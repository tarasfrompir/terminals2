...
function send_message($terminalname, $message, $terminal) {
    ...
    try {
        if ((!defined('SETTINGS_SPEAK_SIGNAL') OR SETTINGS_SPEAK_SIGNAL == '1') AND (int)time() - (int)strtotime($terminal['LATEST_REQUEST_TIME']) > 30 AND $tts AND $tts_setting['TTS_DINGDONG_FILE'] AND $tts->play_media(ROOT.'cms/cached/sounds/'.$tts_setting['TTS_DINGDONG_FILE'])) {
