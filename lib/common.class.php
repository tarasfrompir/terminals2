...
function sayTo($ph, $level = 0, $destination = '')
{
    ...
    foreach ($terminals as $terminal) {
        ...
        } else if ($rec['IMPORTANCE'] >= $terminal['MIN_MSG_LEVEL'] AND $terminal['LINKED_OBJECT'] AND $terminal['IS_ONLINE'] AND $terminal['CANTTS']) {
    ...
    if ($needgenerateaudio) {
        processSubscriptionsSafe('SAYTO', array(
            'level' => $level,
            'message' => $ph,
            'member_id' => $member_id
        )); //, 
...
function say($ph, $level = 0, $member_id = 0, $source = '')
{
    ...
    if ($needgenerateaudio) {
        processSubscriptionsSafe('SAY', array(
            'level' => $level,
            'message' => $ph,
            'member_id' => $member_id
        )); //, 'ignoreVoice'=>$ignoreVoice
    }
...
function processCommand($command)
{
    ...
    if (!$pattern_matched) {
        getObject("ThisComputer")->callMethod("commandReceived", array(
            "command" => $command
        ));
    }
}
