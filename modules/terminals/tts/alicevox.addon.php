...
class alicevox extends tts_addon
{
    ...
    public function say_media_message($message, $terminal) //SETTINGS_SITE_LANGUAGE_CODE=код языка
    {
       if ($message['CACHED_FILENAME']) {
            ...
            if ($this->dingdong) {
                shell_exec(PATH_TO_FFMPEG . ' -i ' . ROOT . "cms/sounds/" . $this->dingdong . ' -i ' . $message['CACHED_FILENAME'] . ' -filter_complex concat=n=2:v=0:a=1 -f WAV -acodec pcm_s16le -ac 1 -ar 22000  -vn -y ' . $filename);
            } else {
