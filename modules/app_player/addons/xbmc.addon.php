...
class xbmc extends app_player_addon
{
    ...
    function status()
    {
        ...
        if ($this->kodi_player_id()) {
            ...
            // Track ID, Length
            if ($this->kodi_request('Player.GetItem', array(
                'playerid' => $player_id,
                'properties' => array(
                    'duration'
                )
            ...
            // Volume
            if ($this->kodi_request('Application.GetProperties', array(
                'properties' => array(
                    'volume'
                )
            ))) {
            ...
            if ($this->kodi_request('Player.GetProperties', array(
                'playerid' => $player_id,
                'properties' => array(
                    'speed',
                    'time',
    ...
    function play($input)
    {
        $this->reset_properties();
        if (strlen($input)) {
            if ($this->kodi_request('Player.Open', array(
                'item' => array(
                    'file' => $input
                )
            ))) {
    ...
    function pause()
    {
        if ($this->kodi_player_id()) {
            $player_id = $this->data;
            if ($player_id != -1) {
                if ($this->kodi_request('Player.PlayPause', array(
                    'playerid' => $player_id
                ))) {
                    $this->success = TRUE;
                    $this->message = 'OK';
    ...
    function stop()
    {
        if ($this->kodi_player_id()) {
            $player_id = $this->data;
            if ($player_id != -1) {
                if ($this->kodi_request('Player.Stop', array(
                    'playerid' => $player_id
                ))) {
                    $this->success = TRUE;
                    $this->message = 'OK';
    ...
    function next()
    {
        if ($this->kodi_player_id()) {
            $player_id = $this->data;
            if ($player_id != -1) {
                if ($this->kodi_request('Player.GoTo', array(
                    'playerid' => $player_id,
                    'to' => 'next'
                ))) {
                    $this->success = TRUE;
        ...
        if (strlen($position)) {
            if ($this->kodi_player_id()) {
                ...
                if ($player_id != -1) {
                    ...
                    if ($this->kodi_request('Player.Seek', array(
                        'playerid' => $player_id,
                        'value' => array(
                            'hours' => $hours,
                            'minutes' => $minutes,
    ...
    function set_volume($level)
    {
        $this->reset_properties();
        if (strlen($level)) {
            if ($this->kodi_request('Application.SetVolume', array(
                'volume' => (int) $level
            ))) {
                $this->success = TRUE;
                $this->message = 'OK';
    ...
    function pl_get()
    {
        if ($this->kodi_playlist_id()) {
            ...
            if ($playlist_id != -1) {
                if ($this->kodi_request('Playlist.GetItems', array(
    ...
    function pl_add($input)
    {
        ...
        if (strlen($input)) {
            if ($this->kodi_playlist_id()) {
                $playlist_id = $this->data;
                if ($playlist_id != -1) {
                    if ($this->kodi_request('Playlist.Add', array(
                        'playlistid' => $playlist_id,
                        'item' => array(
                            'file' => $input
                        )
        ...
        if (strlen($id)) {
            if ($this->kodi_track_position($id)) {
                ...
                if ($this->kodi_playlist_id()) {
                    $playlist_id = $this->data;
                    if ($playlist_id != -1) {
                        if ($this->kodi_request('Playlist.Remove', array(
                            'playlistid' => $playlist_id,
    ...
    function pl_empty()
    {
        if ($this->kodi_playlist_id()) {
            $playlist_id = $this->data;
            if ($playlist_id != -1) {
                if ($this->kodi_request('Playlist.Clear', array(
                    'playlistid' => $playlist_id
                ))) {
                    $this->success = TRUE;
                    $this->message = 'OK';
    ...
    function pl_random()
    {
        if ($this->kodi_player_id()) {
            $player_id = $this->data;
            if ($player_id != -1) {
                if ($this->kodi_request('Player.SetShuffle', array(
                    'playerid' => $player_id,
                    'shuffle' => 'toggle'
                ))) {
                    $this->success = TRUE;
        ...
        if ($this->kodi_player_id()) {
            ...
            if ($player_id != -1) {
                if ($this->kodi_request('Player.GetProperties', array(
                    ...
                    if ($this->kodi_request('Player.SetRepeat', array(
                        'playerid' => $player_id,
                        'repeat' => $repeat
                    ))) {
                        $this->success = TRUE;
